...
DROP POLICY IF EXISTS "Anyone can view incidencias" ON public.incidencias;
DROP POLICY IF EXISTS "Anyone can insert incidencias" ON public.incidencias;
DROP POLICY IF EXISTS "Anyone can update incidencias" ON public.incidencias;
...
CREATE POLICY "Operarios view own incidencias" ON public.incidencias
FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Staff view all incidencias" ON public.incidencias
FOR SELECT TO authenticated USING (public.is_staff(auth.uid()));
CREATE POLICY "Authenticated insert own incidencias" ON public.incidencias
FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Staff update incidencias" ON public.incidencias
...
CREATE POLICY "Operarios view own reparaciones" ON public.reparaciones
FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Staff view all reparaciones" ON public.reparaciones
FOR SELECT TO authenticated USING (public.is_staff(auth.uid()));
CREATE POLICY "Preparacion users insert reparaciones" ON public.reparaciones
FOR INSERT TO authenticated WITH CHECK (
  auth.uid() = user_id
  AND EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.puesto = 'preparacion_molde')
);
CREATE POLICY "Staff update reparaciones" ON public.reparaciones
FOR UPDATE TO authenticated USING (public.is_staff(auth.uid()));

INSERT INTO storage.buckets (id, name, public)
VALUES ('incidencias-fotos', 'incidencias-fotos', true)
ON CONFLICT (id) DO NOTHING;

CREATE POLICY "Public read incidencia photos" ON storage.objects
FOR SELECT USING (bucket_id = 'incidencias-fotos');
